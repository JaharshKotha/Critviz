...
//                 }
                    ...
                    console.log(this.crit_comparer)
...
RainbowGraph.prototype.buildChart = function () {
    ...
    scoreBar = this.svg.selectAll(".scoreBar")
        ...
        .on("mouseover", function (d) {
            ...
            stags = _this.svg.selectAll(".stu_names");
            console.log(stags[0][0])
            ih=0;

            for(ih=0;ih < stags[0].length;ih++)
            {
                if(parseInt(stags[0][ih].getAttribute("stu_id")) == crit_data[0])
                {
                    console.log(stags[0][ih].getAttribute("stu_id"))
